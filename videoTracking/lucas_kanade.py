...
#get from webcam
cap = cv2.VideoCapture("football_stitched.mp4")
    
#find params for corner detection via Shi-Tomasi
shi_params = dict( maxCorners = 500,
                       qualityLevel = 0.01, #determines amount of relevant points determined
                       minDistance = 10,
                       blockSize = 10 )
...
while(1):
	...
	p_old = cv2.goodFeaturesToTrack(old_gray[0:len(old_gray)-50, 0:len(old_gray[0])], mask = None, **shi_params)
	...
	writer.write(img)
	cv2.imwrite("im"+str(index)+".png", img)
